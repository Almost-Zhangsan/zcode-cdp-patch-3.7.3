...
var FALLBACK_MANIFEST = {
  ...
  objects: {
    ...
    Browsers: {
      members: ["list", "get", "getDefault", "getForUrl"].map((name) => ({
        name,
        kind: "method",
        signature: `${name}(...)`,
      })),
    ...
    PlaywrightAPI: {
      members: [
        ...
        },
        {
          ...
          kind: "method",
          signature:
            "waitForEvent(event, options?): Promise<PlaywrightDownload>",
          command: "playwright",
          declarations: [
            {
              signature:
                'waitForEvent(event: "download", options?): Promise<PlaywrightDownload>',
            },
            {
              signature:
...
function capabilityIds(descriptor, scope) {
  return new Set(
    (descriptor.capabilities[scope] ?? []).map((capability) => capability.id),
  );
}
var BrowserApiPolicy = class {
  ...
  supports(objectName, memberName) {
    ...
    if (supported && member.declarations?.length) {
      supported = member.declarations.some((declaration) =>
  ...
  supportsRequirement(requirement) {
    if (
      (requirement.unsupportedByDefaultIn ?? []).includes(this.descriptor.type)
    )
      return false;
    return !requirement.requiresCapabilities?.some((capability) => {
      ...
      return (
        !this.browserCapabilities.has(capability) &&
        !this.tabCapabilities.has(capability)
      );
    });
  }
};
function createBrowserApiProxy(target, objectName, policy, options = {}) {
  const isHidden = (property) => {
    ...
    return known
      ? !policy.supports(objectName, property)
      : options.hideUnknown === true;
  };
  const handler = {
    get(current, property, receiver) {
      if (isHidden(property)) return void 0;
      const value = Reflect.get(current, property, receiver);
      return options.hideUnknown && typeof value === "function"
        ? value.bind(current)
        : value;
    },
...
function documentApplies(document, descriptor, api) {
  ...
  if (
    when.requiredBrowserCapabilities?.some((id) => !browserCapabilities.has(id))
  )
    return false;
    ...
    if (
      when.requiredApiMembers.some((member) => {
        ...
        return (
          separator <= 0 ||
          !policy.supports(
            member.slice(0, separator),
            member.slice(separator + 1),
          )
        );
      })
    ) {
                ...
                ...(member.declarations
                  ? {
                      declarations: member.declarations.filter(
                        (declaration) =>
                          (declaration.unsupportedByDefaultIn?.length ?? 0) ===
                            0 &&
...
function loadBrowserDocumentation(documentationRoot, name, descriptor) {
  ...
  if (name) {
    ...
    const markdown = document?.path
      ? readMarkdown(documentationRoot, document.path)
      : void 0;
  ...
  for (const doc of manifest?.documents ?? []) {
    if (
      !doc.path ||
      doc.mode === "lookup" ||
      !documentApplies(doc, descriptor, api)
    )
      continue;
...
function urlMatchRank(target, candidate) {
  ...
  const isParentHost = (parent, child) =>
    parent.includes(".") && isIP(parent) === 0 && child.endsWith(`.${parent}`);
  if (
    isParentHost(candidateHost, targetHost) ||
    isParentHost(targetHost, candidateHost)
  )
    return 3;
  return void 0;
}
  ...
  const matches = infos.flatMap((info) => {
    ...
    for (const value of tabsByBrowserId.get(info.id) ?? []) {
      try {
        ...
        if (rank !== void 0 && (best === void 0 || rank < best)) best = rank;
      } catch {}
    }
    ...
    throw new BrowserCommandError(
      command,
      {
        ...
        error: {
          ...
          message: `Browser result missing ${payloadName}`,
        },
      },
      "execution_error",
    );
...
var defaultObjectWrapper = (value, objectName) =>
  new Proxy(value, {
    get(target, property) {
      if (
        typeof property === "string" &&
        !publicMembers[objectName]?.has(property)
      ) {
        return void 0;
      }
...
function normalizeSelections(input) {
  ...
  return values.map((selection) => {
    ...
    if (
      selection.value === void 0 &&
      selection.label === void 0 &&
      selection.index === void 0
    ) {
      throw new Error(
...
var PlaywrightLocator = class _PlaywrightLocator {
  ...
  create(selector) {
    return this.wrap(
      new _PlaywrightLocator(this.run, this.owner, selector, this.wrap),
      "PlaywrightLocator",
    );
  }
  [locatorDetails]() {
  ...
  async click(options = {}) {
    try {
      ...
      throw withContext(
        error,
        `waiting on click for selector ${this.selector}`,
      );
    }
  }
  async dblclick(options = {}) {
    try {
      ...
      throw withContext(
        error,
        `waiting on dblclick for selector ${this.selector}`,
      );
    }
  }
  async selectOption(input, { timeoutMs } = {}) {
    try {
      await this.action("selectOption", {
        selections: normalizeSelections(input),
        timeoutMs,
      });
    } catch (error) {
      throw withContext(
  ...
  async fill(value, { timeoutMs } = {}) {
    ...
    try {
      ...
      throw withContext(
        error,
        `locator.fill failed for selector ${this.selector}`,
      );
    }
  }
  async type(value, { timeoutMs } = {}) {
    ...
    try {
      ...
      throw withContext(
        error,
        `locator.type failed for selector ${this.selector}`,
      );
    }
  }
  async press(value, { timeoutMs } = {}) {
    ...
    try {
      ...
      throw withContext(
        error,
        `locator.press failed for selector ${this.selector}`,
      );
    }
  }
  ...
  async downloadMedia({ timeoutMs } = {}) {
    try {
      ...
      throw withContext(
        error,
        `locator.downloadMedia failed for selector ${this.selector}`,
      );
    }
  }
  ...
  and(locator) {
    ...
    return this.create(
      `${this.selector} >> internal:and=${JSON.stringify(selector)}`,
    );
  }
  or(locator) {
    const selector = this.compatibleSelector(locator, "locator.or");
    return this.create(
      `${this.selector} >> internal:or=${JSON.stringify(selector)}`,
    );
  }
  filter(options = {}) {
    ...
    if (options.has !== void 0) {
      const selector = this.compatibleSelector(
        options.has,
        "locator.filter has",
      );
...
var PlaywrightAPI = class {
  ...
  async waitForEvent(event, options = {}) {
    ...
    return event === "download"
      ? this.wrap(
          new PlaywrightDownload(this.run, value.id),
          "PlaywrightDownload",
        )
      : this.wrap(
          new PlaywrightFileChooser(
            this.run,
            value.id,
            Boolean(value.isMultiple),
          ),
          "PlaywrightFileChooser",
        );
  }
  ...
  async elementScreenshot(options) {
    ...
    const result = await runAction(this.run, {
      name: "elementScreenshot",
      ...options,
    });
...
var BrowserCapabilityCollection = class {
  ...
  async get(id) {
    ...
    if (id === "visibility" && this.execute) {
      return new VisibilityBrowserCapability(
        capability,
...
function validateViewportSize(viewportSize) {
  if (
    ...
    throw new TypeError(
      `setViewportSize requires integer width ${BROWSER_VIEWPORT_LIMITS.minWidth}..${BROWSER_VIEWPORT_LIMITS.maxWidth} and height ${BROWSER_VIEWPORT_LIMITS.minHeight}..${BROWSER_VIEWPORT_LIMITS.maxHeight}`,
    );
  }
}
...
var Tab = class {
    ...
    return this.wrapObject(
      {
        ...
        downloadMedia: ({ node_id }) =>
          this.action({ method: "click", ref: node_id }),
        type: ({ text }) => this.action({ method: "type", text }),
        scroll: ({ node_id, x, y }) =>
          this.action({
            method: "domCuaScroll",
  ...
  get cdp() {
    // 不走 wrapObject/hideUnknown：CdpAPI 不在出厂 api.json 白名单，包一层方法会变成 undefined
    const self = this;
    const runCdp = (payload) => self.run({ method: "cdp", ...payload });
    const api = {
      async send(method, params = {}) {
        const result = expectOk(
          { method: "cdp", op: "send", cdpMethod: method, params },
          await runCdp({ op: "send", cdpMethod: method, params }),
        );
        return result.value ?? null;
      },
      async events(options = {}) {
        const result = expectOk(
          { method: "cdp", op: "events" },
          await runCdp({
            op: "events",
        ...
        return result.value ?? { events: [], count: 0, totalBuffered: 0 };
      },
      async enableDebugger() {
        const enabled = await api.send("Debugger.enable", {});
        try {
          await api.send("Debugger.setSkipAllPauses", { skip: false });
        } catch {}
        return enabled;
      },
      ...
      async getCallStack() {
        const { events } = await api.events({ clear: false, limit: 500 });
      ...
      async resume() {
        return api.send("Debugger.resume", {});
      },
      async pause() {
        return api.send("Debugger.pause", {});
      },
      async setBreakpointByUrl(options) {
        return api.send("Debugger.setBreakpointByUrl", options);
      },
      async removeBreakpoint(breakpointId) {
        return api.send("Debugger.removeBreakpoint", { breakpointId });
      },
      async networkEnable() {
        return api.send("Network.enable", {});
      },
      async runtimeEnable() {
        return api.send("Runtime.enable", {});
      },
      async openDevTools() {
        const result = expectOk(
          { method: "cdp", op: "openDevTools" },
          await runCdp({ op: "openDevTools" }),
        );
        return result.value ?? { opened: true };
      },
    };
    return api;
  }
  async openDevTools() {
    const result = expectOk(
      { method: "cdp", op: "openDevTools" },
      await this.run({ method: "cdp", op: "openDevTools" }),
    );
    return result.value ?? { opened: true };
  }
};
var BrowserTabs = class {
  ...
  async selected() {
    ...
    const selected =
      summaries.find((summary) => summary.active === true) ?? summaries.at(-1);
    return selected ? this.tab(selected) : void 0;
  }
  async get(tabId) {
    const summary = (await this.summaries()).find(
      ...
      throw new BrowserCommandError(
        { method: "list" },
        {
          ...
          error: {
            ...
            message: `Browser tab '${tabId}' is unavailable`,
          },
        },
        "backend_unavailable",
      );
...
var Browser = class {
  ...
  constructor(
    ...
    this.execute = (command) =>
      execute(this.info.id, this.info.generation, command);
    const capabilityDocumentation = (name) =>
      this.readDocumentation(name, this.info);
    const wrapTab = (tab) =>
      createBrowserApiProxy(
        tab.applyPlaywrightPolicy(this.policy),
        "Tab",
        this.policy,
      );
    this.capabilities = new BrowserCapabilityCollection(
  ...
  createTab(tabId) {
    return createBrowserApiProxy(
      new Tab(
        ...
        void 0,
        this.info.capabilities.tab ?? [],
        (name) => this.readDocumentation(name, this.info),
      ),
      "Tab",
      this.policy,
    );
...
var BrowsersFacade = class {
  ...
  constructor(transport, options = {}) {
    ...
    this.readDocumentation = (name, descriptor) => {
      assertAvailable();
      return loadBrowserDocumentation(
    ...
    this.runtimeObject = createBrowserApiProxy(
      this,
      "Browsers",
      topLevelPolicy,
      {
  ...
  async open(url) {
    const tab = await (await this.getDefault()).tabs.new();
    if (url) {
      await tab.goto(url);
    }
    return tab;
  ...
  tab(tabId) {
    ...
    return new Tab(
      async (command) => (await browser).executeCommand(command),
      tabId,
    );
  }
  documentation(name) {
...
var NODE_REPL_BROWSER_BRIDGE_SYMBOL = Symbol.for(
  "zcode.node-repl.browser-control-bridge",
);
...
export { setupBrowserRuntime2 as setupBrowserRuntime };
